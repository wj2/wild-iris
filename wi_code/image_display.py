...
def get_images(
    folder, template=".*\.info$", image_folder="pictures", info_key="picture"
):
...
def resave_images(
    ...
    for fl in fls:
        ...
        if m is not None:
            ...
            crop_img = img.crop(
                box=(box_lb[0], box_lb[1], box_lb[0] + h_len, box_lb[1] + v_len)
            )
...
def plot_img(
    ...
    ax_img.imshow(img)
    box_lb, h_len, v_len, _ = get_crop_box(
        info,
        img.size,
    ...
    rect = patches.Rectangle(
        box_lb, h_len, v_len, linewidth=1, edgecolor="r", facecolor="none"
    )
